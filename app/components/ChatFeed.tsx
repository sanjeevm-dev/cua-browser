      ...
      if (initialMessage && !agentStateRef.current.sessionId) {
        ...
        try {
          ...
          if (!sessionData.success) {
            // Show a user-friendly error step instead of throwing
            const reason = sessionData.error as string;
            const code = (sessionData.code as string) || "session_error";

            const message =
              code === "browser_minutes_exhausted"
                ? `Your free Browserbase minutes are exhausted. Please upgrade your plan to continue.\n\n${sessionData.upgradeUrl ?? "https://browserbase.com/plans"}`
                : reason || "Failed to create session. Please try again.";

            const errorStep: BrowserStep = {
              text: message,
              reasoning: "Session initialization error",
              tool: "MESSAGE",
              instruction: "",
              stepNumber: (agentStateRef.current.steps?.length || 0) + 1,
            };
            ...
            setIsLoading(false);
            return; // Stop further initialization when session fails
          }
              ...
              {initialMessage && (
                <div className="relative">
                  ...
                  >
                    ...
                    <p className="font-semibold pr-6 text-white">Goal:</p>

                    <p className="break-words overflow-hidden text-ellipsis max-w-full text-white/90">
                      {initialMessage}
                    </p>
                  </motion.div>
                </div>
                        ...
                        <span
                          className={`px-2 py-1 ${
                            isSystemMessage
                              ? "text-white/90"
                              : "text-white/80"
                          } border border-white/[0.08] text-xs rounded`}
                        >
                          {step.tool}
                        </span>
                      </div>
                      ...
                      if (questionText) {
                        return (
                          <motion.div
                            ...
                            className={`p-4 bg-blue-600/20 backdrop-blur-xl text-white border border-white/[0.08] rounded-xl font-ppsupply space-y-2 mt-2`}
                          >
                            <div className="flex justify-between items-center">
                              {/* <span className="text-sm text-white/70">
                              {isOnlyQuestion ? "Question" : "Follow-up"}
                            </span> */}
                              {/* <span className="px-2 py-1 text-white/70 rounded text-xs">
                              QUESTION
                            </span> */}
                            </div>
                            <div className="font-medium">
                              <div className="p-2 border-l-2 border-blue-500/50">
                                <span className="text-white/90">{questionText}</span>
                              </div>
                            </div>
                          </motion.div>
                        );
              ...
              {isWaitingForInput && !isAgentFinished && (
                ...
                >
                  <input
                    ...
                    className="flex-1 px-2 sm:px-4 py-2 bg-black/[0.4] backdrop-blur-xl text-white placeholder:text-white/40 border border-white/[0.08] focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent font-ppsupply transition-all text-sm sm:text-base rounded-lg"
                  />
                  <button
                    type="submit"
                    disabled={!userInput.trim()}
                    className="px-2 sm:px-4 py-2 bg-blue-600 text-white font-ppsupply disabled:opacity-50 disabled:cursor-not-allowed hover:bg-blue-700 transition-colors text-sm sm:text-base whitespace-nowrap rounded-lg"
                  >
                    Send
                  </button>
                </motion.form>
